 ...
 *
 * @param {any} locality
 * @param {any} [matchResult]
 * @returns {import('./localityIntelligenceContract.ts').NormalizedLocalityIntelligence}
 */
export function normalizeLocalityIntelligence(locality, matchResult) {
    ...
    const rawCriteria = loc.publishedScore?.criteriaScores
        ?? loc.intelligence?.areaScore?.criteriaScores
        ?? loc.scoring?.criteria
        ?? match.criteriaScores
        ?? null;

    const scoreCriteria = rawCriteria
        ? {
